...
describe('確度の軸（C-127）', () => {
  ...
  test('確度の線は、規則が無ければ引かない（タイルは記入された人数を出す）', async () => {
    // ★ 0017 の「無いことを 0 と書かない」は生きている。
    //   ただし 0039（C-151）で確度は**人が記入するもの**になった ――
    //   規則が無いのは欠落ではなく正しい状態なので、
    //   タイルに「算出なし」と出し続けるのは**古い言い方**である（C-206）。
    //   線（時系列）は算出の産物なので、これまでどおり規則を見る。
    ...
    assert.ok(!/derived=\{hasRules\}/.test(src),
      'タイルがまだ「算出なし」を出している（確度は記入するものになった）')
