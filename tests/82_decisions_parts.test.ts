...
describe('設計判断の1件1ファイル（db/decisions/）', () => {
  ...
  // ★ 2026-08-19（C-227）: 重複3件（C-45 / C-46 / C-116）を解消した。
  //   旧版はここで `dupes.length > 0` を要求しており、**解消した瞬間に落ちる**
  //   作りだった（「重複が解消されたなら、この検査は消してよい」と自ら書いていた）。
  //
  //   ★★ だが**消すだけにしない。** 消すと、重複が再発したときに誰も気づかない。 ★★
  //   向きを逆にして**増えないこと**を見る。生成側の競合告知は、
  //   万一の再発時にも働く必要があるので、そのときだけ検査する形で残す。
  test('★ 番号の重複は無い。あるなら、両方を1つのファイルに載せて競合を告げている', async () => {
    const dupes = [...new Set(entries.map((e) => e.id))]
      .filter((id) => entries.filter((e) => e.id === id).length > 1)

    // 再発したときは、生成物が黙って片方を捨てていないことを確かめる。
    ...
    // ★ そのうえで、**重複そのものを増やさない。**
    //   番号はコード内コメントとコミットメッセージから参照される。
    //   git 履歴は後から直せないので、重複を作った時点で参照先が永久に定まらなくなる。
    assert.deepEqual(dupes, [],
      `番号が重複している ―― 後から書いた側に新番号を振ること（C-227 と同じ手当て）`)
  })
