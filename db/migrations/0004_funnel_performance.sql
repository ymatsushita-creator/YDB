...
--
-- 発見の経緯: 実データ（4年度・応募947件・接点6154件）で計測したところ
--
--     f_funnel_daily(90) 全年度   35,713ms   489行
--
-- ダッシュボードの主画面が35秒かかる。原典の意味論は正しいが実用にならない。
-- 原因は2つあり、順に潰した。
--
-- (1) 相関副問い合わせの繰り返し評価 …… 35.7秒 → 1.29秒
--     返す489行のそれぞれが6本の相関副問い合わせを走らせ、うち4本が
--     v_effective_status_histories（訂正チェーンを解く再帰CTE）を
--     先頭から評価し直していた。489 × 4 回、同じ再帰を繰り返していた。
--     → 訂正の解決は1回だけ。応募ごとに節目の日を1行へ畳み、
--       日次の増分を出してウィンドウ関数で累積する。
--
-- (2) 林の日ごとの重複排除 …… 1.29秒 → 81ms
--     切り分けた内訳:
--         日次系列だけ            5ms
--         林（grove）だけ     1,211ms   ← ここ
--         マイルストーン集約     17ms
--     日ごとに count(DISTINCT person_id) を取り直しており、
--     日数 × 窓内の接点数だけ重複排除が走っていた。
...
--       構造的に起きない。
--
-- 合計 35,713ms → 81ms。意味論は変えていない。
-- 同じ数字を返すことを tests/07_funnel_equivalence.test.ts で
-- 原典の定義と1行ずつ突き合わせている。
-- =============================================================
