...
-- という形で4箇所に複製されていた。合格の定義が4箇所にあると、
...
--
-- 0003 に循環を検出するトリガは無い。代わりに status_histories を
-- 追記専用にしてある。corrects_history_id は既存の行しか指せないので、
-- INSERT だけで作られるグラフの辺は必ず過去向きになり、循環はそもそも
-- 作れない。循環が生まれるのは UPDATE で辺を張り替えたときだけで、
-- その UPDATE を禁じてある。検出ではなく、生まれる余地を閉じている。
