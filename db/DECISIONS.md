...
### A-8. ダッシュボードの主画面が35秒かかっていた ★影響大

実データ（4年度・応募947件・接点6154件）で計測した結果:

| クエリ | 変更前 | 変更後 |
|---|---|---|
| `f_funnel_daily(90)` 全年度 | **35,713ms** | **81ms** |
| `v_application_state` | 1,148ms | 9ms |
| `v_person_season_state` | 1,192ms | 31ms |

原典の意味論は正しいが、そのままでは画面が出ない。原因は2つ。

**(1) 相関副問い合わせの繰り返し評価。** 返す489行のそれぞれが6本の相関
副問い合わせを走らせ、うち4本が `v_effective_status_histories`
（訂正チェーンを解く再帰CTE）を先頭から評価し直していた。同じことを
489 × 4 回やっていた。`v_application_state` も応募1件につき EXISTS を3本、
同様に再帰を繰り返していた。
...
**(2) 林の日ごとの重複排除。** 残った 1.2 秒はすべてここだった。
...
意味論を変えていないことは、原典の素朴な実装を参照として残し、
ランダムな敵対的データ（二重訂正・両種の無効化・合格前の辞退・差し戻し・
年度外の遷移・削除済み Person・期間の重なる年度）で1行ずつ突き合わせて
確認している。→ `tests/07_funnel_equivalence.test.ts`
