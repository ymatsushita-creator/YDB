...
- `'use client'` は**表（`_components/sheet.tsx`）だけ**（C-95 / C-104）。増やさない。
- 意匠の正典は `basic/DESIGN.md`。色・余白・角丸をここで決めない。
- CSS は `_styles/01〜06` に分割済み。**取り込み順＝カスケード順**なので、番号を入れ替えない（`base.css` が `@import` する）。
- 意匠の層は `tokens.css` と `base.css` の**2枚だけ**（C-236）。白黒の土台・ブランド線・ガラスの3枚は退役した。**面をスペクトラムで塗らない。**
