...
/**
 * ボーダーライン（誰を通すか）の問い合わせ。
 *
 * ★ ファイル名が `cockpit.ts` のままなのは意図的である。
 *   実行⑨で画面の名前が「運転席」から「ボーダーライン」に変わったが、
 *   **凍結された報告書 32 箇所がこのファイル名を指している。**
 *   報告書は凍結する規約なので直せない。ファイル名を変えると、
 *   その 32 箇所が存在しないファイルを指すことになる。
 *   画面に出る言葉だけを変え、内部の名前は据え置く。
 *
...
// ★ getCommunityMap と CommunityMapRow は実行⑨で削除した。
//   旧ボーダーライン（運転席）の「アプローチ可能圏」パネル専用で、
//   画面を作り直したあと**どこからも呼ばれなくなった**（テストも無かった）。
//   構成コミュニティを画面に戻すなら、ここではなく新しい画面の要件から始める。
