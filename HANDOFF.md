...
| # | 何を | 誰が | 現在の影響 |
|---|---|---|---|
| 1 | 〜〜**欠番45件**を `db/DECISIONS.md` へ書く〜〜 **解消**（C-223）―― コード内コメントから41件を移設。残り4件（C-171 / C-173 / C-182 / C-186 / D-30）は手がかり不足として「不足」と明記した | ―― | ―― |
...
5 は AI が決められない（どちらを正とするかは判断そのもの）。
2 は封印対象の編集を伴い、pre-commit が止める。**許可があっても機械は止まる。**

★ **1 は解消した（C-223）。**「AIが埋められない」は事実に反していた。
実行⑯⑰は記録を残さなかったのではなく、**`db/DECISIONS.md` ではなく
コード内のコメントに書いていた。** 過半（24件）は `app/_styles/10-round-16-17.css`
1本に集中しており、依頼者の発言そのものまで残っていた。

**創作ではなく移設なので AI が起こせた。** 41件を移設し、欠番は 45 → 0 件。
手がかりが1行しか無い4件（C-171 / C-173 / C-182 / C-186 / D-30）は
**埋めずに「不足」と明記した** ―― 推測で埋めない。

## 2026-08-19 の構成改修（C-221 / C-222）

- **意匠の正典を `basic/DESIGN.md` に一本化**（C-221）。生成器 `build-tokens.ts` を
  新しい語彙（gradients・18種のプロパティ）へ対応させた。
- **多層序列を廃止し、権限を `Hitler.md` の1段へ集約**（C-222）。
  `SUPERVISOR.md` と `vision.md` を畳み、ルート規律文書は 10本 → 8本。
  S1〜S13 は規律から**診断**へ格下げ（S7 は残す）。
- **`.audit/MANIFEST.sha256` の再封印がもう一度要る。** 封印は改修前の状態で焼かれている。

```
/Volumes/KIOXIA_2TB/NEO/金融庁の黒崎です！/bin/kurosaki install --repo .
```
