...
| ④ 次に何をすべきか | **導出で成立**（0012） | `v_open_tasks` が4種のやることを既存の事実から出す（C-17） | `/cockpit` に一覧があり、**「担当を決める」は画面から実行できる**（C-20） |

④は**導出で成立させた。** Task の記録層は無いままで（`domain.md` 10-2 は
未決）、既存の事実から出せるやることだけを出している。
**手で作るタスク（連絡する・催促する）は表せない**（D-12 の TODO(MVP) 2）。

**書き込みは4種のうち3種（担当を決める・保留を解く・担当を替える）が
入っている**（C-20 / C-21 / C-22）。
書き込みの型は決まった ―― 判定は `src/commands/`（テストから呼べる場所）、
受け渡しは `'use server'`、表示は素の `<form action={...}>`。
**`'use client'` は0件のまま。** 残るのは「評価する」だけで、
これは点と根拠の複数行書き込みになるため大きい。
